...
class Trans_main:
    ...
    def insert_values(self):
        stat_trans=f"select vehicle_id,toll_id,date,time,vehicle_category,fare from transaction"
        cr.execute(stat_trans)
        result=cr.fetchall()
        x = []
        for row in result:
            lst = list(row)
    ...
    def __init__(self):
        ...
        self.root.title("Toll fare transaction details")
        self.root.iconbitmap('D:\\final toll tax system\\TollTaxSys\\icons\\tollpanel.ico')
        self.label_frame=LabelFrame(self.root,text='Daily Transactions Record',borderwidth=3,bd=5,labelanchor=NW,font=('Helvetica',42,'bold')
        ...
        col=('Vehicle id','Toll id','Date','Time','Vehicle Category','Fare amount')
        ...
        self.tree.column('Vehicle Category',width=180)
        self.tree.column('Fare amount',width=180)
